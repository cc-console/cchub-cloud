...
### Ubuntu 服务器(无界面)
用你专属的 `你的名字.cchub.cloud` 地址,从手机直接访问一台远程服务器:
```bash
# 1) 拿设备码:到 https://cc.cchub.cloud 注册 → 设一个名字 → 点「生成设备码」
# 2) 在服务器上:
...
```
完整服务器指南(代理、cloudflared、systemd 常驻):**[cc.cchub.cloud/#server](https://cc.cchub.cloud/#server)**。
